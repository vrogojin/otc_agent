...
const dealId = 'f4584f67a5a12020c2b7e3138de2e610';
...
try {
  ...
  const queueItems = db.prepare(`
    SELECT id, purpose, status, asset, amount, chainId, toAddr, submittedTx
    FROM queue_items
    WHERE dealId = ?
  ...
  queueItems.forEach(item => {
    console.log(`\n${item.purpose} (${item.status}): ${item.amount} ${item.asset} on ${item.chainId}`);
    console.log(`  To: ${item.toAddr}`);
    if (item.submittedTx) {
